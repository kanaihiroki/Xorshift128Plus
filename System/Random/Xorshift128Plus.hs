...
-- Read <http://xorshift.di.unimi.it/xorshift128plus.c
-- original implementation and description> is strictly recommended.
--
...
-- @
--
-- Produce a random value using 'next' or 'next01':
--
-- @
--   let v = 'next' gen
-- @
...
-- | Generate 64bit random value and next generator state.
next :: Gen -> (Word64, Gen)
next g = let (# n, g' #) = next# g
         in  (n, g')
{-# INLINE next #-}

-- | Generate random value between 0 and 1. And next generator state.
next01 :: Gen -> (Double, Gen)
next01 g = let (# n, g' #) = next# g
           in (fromIntegral n / word64max, g')
{-# INLINE next01 #-}

-- | Same as 'next', but return value with unboxed tuple.
next# :: Gen -> (# Word64, Gen #)
next# g = let g'@(Gen s0 s1) = step g
          in  (# (s0 + s1), g' #)
{-# INLINE next# #-}

-- | Create generator by seed value. Please do not supply 0.
